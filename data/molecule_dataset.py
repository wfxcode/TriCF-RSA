...
class Molecule_dataset(InMemoryDataset):
    ...
    def process(self):
        ...
        for index, row in self.df.iterrows():
                ...
                data.m_id = row['Molecule_ID']
                data.smiles = row['SMILES']

                # 药物3D数据
                # data_3d = Data()
                file_3d = f"data/R-SIM/R-SIM_drug_3D/{row['Molecule_ID']}"
                ...
                # 药物序列emb
                file_seq = f"data/R-SIM/R-SIM_drug_seq/{row['Molecule_ID']}.pt"
                atom_emb = torch.load(file_seq, map_location='cpu').detach().clone().float()[0]  # 89
                ...
                # data_2d = Data()
                file_2d = f"data/R-SIM/R-SIM_drug_2D/{row['Molecule_ID']}.pt"
