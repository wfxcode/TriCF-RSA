...
from data import IC50_RNA_dataset, IC50_Molecule_dataset
...
rna_dataset = IC50_RNA_dataset()
molecule_dataset = IC50_Molecule_dataset()
...
if __name__ == '__main__':
    ...
    # 创建保存目录
    save_dir = os.path.join('save', current_file_name, current_time + f"_{n_splits}")
    ...
    kf = regressor_stratified_cv(n_splits=n_splits, n_repeats=1, random_state=seed_dataset, group_count=10, strategy='uniform')
    ...
    for train_id, test_id in splits:
        ...
        train_loader = DataLoader(
            ...
            pin_memory=True, persistent_workers=persistent_workers, prefetch_factor=prefetch_factor  # 预取2个batch
        )
        # train_loader = DataLoader(
        ...
        for epo in range(EPOCH):
            ...
            with torch.set_grad_enabled(False):
                ...
                for step, (batch_rna_test, batch_mole_test) in enumerate(test_loader):
                    # 准备标签
                    label = torch.from_numpy(np.asarray(batch_rna_test.y)).float()
                    # 前向传播
